...
from superset.dhis2.sync_service import schedule_staged_dataset_sync_after_commit
...
class UpdateDatasetCommand(UpdateMixin, BaseCommand):
    ...
    def run(self) -> Model:
        self.validate()
        assert self._model
        dataset = DatasetDAO.update(self._model, attributes=self._properties)

        # If it's a staged local dataset, trigger sync immediately
        try:
            import json
            extra = json.loads(dataset.extra or "{}") if isinstance(dataset.extra, str) else (dataset.extra or {})
            if extra.get("dhis2_staged_local"):
                staged_dataset_id = extra.get("dhis2_staged_dataset_id")
                if staged_dataset_id:
                    try:
                        logger.info(
                            "[DHIS2] Scheduling post-commit sync for updated staged dataset id=%s",
                            staged_dataset_id,
                        )
                        schedule_staged_dataset_sync_after_commit(
                            staged_dataset_id,
                            job_type="scheduled",
                        ...
                        logger.exception(
                            "[DHIS2] Failed to schedule post-commit sync for updated staged dataset id=%s",
                            staged_dataset_id,
                        )
        except Exception:
            pass
    ...
    def validate(self) -> None:
        ...
        # Validate dataset role for metadata update
        if hasattr(self._model, "dataset_role") and self._model.dataset_role:
            try:
                from superset.datasets.policy import DatasetContext, DatasetEligibilityPolicy, DatasetRole
                role = DatasetRole(self._model.dataset_role)
                if not DatasetEligibilityPolicy.is_eligible(role, DatasetContext.METADATA_UPDATE):
                    # Only allow if the user is an admin, per "admin_all"
                    ...
                    if not is_admin:
                        exceptions.append(ValidationError("Cannot edit metadata of a non-metadata dataset."))
            except ValueError:
                pass
