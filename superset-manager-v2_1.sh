...
celery_worker_running() {
  local pid; pid="$(read_pid_file "$CELERY_WORKER_PID_FILE" || true)"
  [[ -n "${pid:-}" ]] && pid_is_running "$pid"
}
celery_beat_running() {
  local pid; pid="$(read_pid_file "$CELERY_BEAT_PID_FILE" || true)"
  [[ -n "${pid:-}" ]] && pid_is_running "$pid"
}
redis_running() {
...
clickhouse_running() {
  curl -fsS "http://$CLICKHOUSE_HOST:$CLICKHOUSE_PORT/ping" >/dev/null 2>&1
}
...
start_redis() {
  ...
  for _ in {1..10}; do sleep 1; redis_running && { ok "Redis started"; return 0; }; done
...
start_frontend() {
  ...
  cd "$FRONTEND_DIR"
  [[ -d "$FRONTEND_DIR/node_modules" ]] || npm install
  local pid
  ...
  for _ in {1..90}; do sleep 2; port_is_in_use "$FRONTEND_PORT" && { ok "Frontend started (PID: $pid)"; echo "  URL: http://$FRONTEND_HOST:$FRONTEND_PORT"; return 0; }; done
...
start_celery_worker() {
  ...
  cd "$BACKEND_DIR"; venv_activate; set_backend_env
  local pid
  ...
  for _ in {1..20}; do sleep 1; celery_worker_running && { ok "Celery worker started (PID: $pid)"; return 0; }; done
...
start_celery_beat() {
  ...
  for _ in {1..20}; do sleep 1; celery_beat_running && { ok "Celery beat started (PID: $pid)"; return 0; }; done
...
clear_logs() {
  ...
  rm -f "$BACKEND_LOG_FILE" "$FRONTEND_LOG_FILE" "$REDIS_LOG_FILE" "$CELERY_WORKER_LOG_FILE" "$CELERY_BEAT_LOG_FILE" "$CLICKHOUSE_LOG_FILE" "$CLICKHOUSE_ERROR_LOG_FILE" "$CLICKHOUSE_STDOUT_LOG_FILE"
  ok "Logs cleared"
}
...
install_system_packages() {
  ...
  if [[ "$CLICKHOUSE_ENABLED" == "1" ]] && ! command_exists clickhouse-client; then
    [[ -f /usr/share/keyrings/clickhouse-keyring.gpg ]] || curl -fsSL https://packages.clickhouse.com/rpm/lts/repodata/repomd.xml.key | sudo gpg --dearmor -o /usr/share/keyrings/clickhouse-keyring.gpg
  ...
  [[ "$CLICKHOUSE_ENABLED" == "1" ]] && sudo systemctl enable --now clickhouse-server || true
...
CLICKHOUSE_STAGING_DATABASE=${CLICKHOUSE_STAGING_DATABASE}
CLICKHOUSE_SERVING_DATABASE=${CLICKHOUSE_SERVING_DATABASE}
CLICKHOUSE_CONTROL_DATABASE=${CLICKHOUSE_CONTROL_DATABASE}
...
EXPOSE_CLICKHOUSE_HTTP=${EXPOSE_CLICKHOUSE_HTTP}
EXPOSE_CLICKHOUSE_NATIVE=${EXPOSE_CLICKHOUSE_NATIVE}
...
configure_firewall_server() {
  ...
  [[ "$EXPOSE_CLICKHOUSE_HTTP" == "1" ]] && sudo ufw allow "${CLICKHOUSE_PORT}"/tcp comment 'ClickHouse HTTP' || true
  [[ "$EXPOSE_CLICKHOUSE_NATIVE" == "1" ]] && sudo ufw allow "${CLICKHOUSE_NATIVE_PORT}"/tcp comment 'ClickHouse native' || true
...
ensure_remote_app_user() {
  ...
  if [[ "$REMOTE_USER" == "root" ]]; then
    ...
    # Create the parent and target paths as root, then hand off ownership
...
rsync_local_code_to_remote() {
  ...
  case "$REMOTE_INSTALL_DIR" in
    /Users/*|/Volumes/*)
      die "REMOTE_INSTALL_DIR points to a local macOS path: $REMOTE_INSTALL_DIR. Set REMOTE_INSTALL_DIR to a Linux server path such as /opt/superset"
      ;;
  esac
...
remote_run_as_app_user() {
  local cmd="$1"
  if [[ "$REMOTE_USER" == "root" && "$REMOTE_SUDO_APP_USER" == "1" ]]; then
    printf '%s\n' "$cmd" | ssh "${SSH_OPTS[@]}" "$(remote_login)" "sudo -H -u '${REMOTE_APP_USER}' bash -s"
  else
    printf '%s\n' "$cmd" | ssh "${SSH_OPTS[@]}" "$(remote_login)" "bash -s"
  fi
}
...
deploy_remote() {
  ...
  prepare_remote_codebase
  push_self_to_remote
  header "Running remote install from synced local code"
  remote_run_as_app_user "
    cd '${REMOTE_INSTALL_DIR}'
    ...
    export EXPOSE_CLICKHOUSE_HTTP='${EXPOSE_CLICKHOUSE_HTTP}'
    export EXPOSE_CLICKHOUSE_NATIVE='${EXPOSE_CLICKHOUSE_NATIVE}'
...
Remote deployment from codebase source:
...
Important environment variables:
  ...
  REMOTE_INSTALL_DIR=\$HOME/${APP_NAME}
  ADMIN_EMAIL=...
  ADMIN_PASSWORD=...
  LETSENCRYPT_EMAIL=...
  POSTGRES_PASSWORD=...
...
Examples:
  CODEBASE_SOURCE=local DOMAIN=supersets.vitalplatforms.com ADMIN_EMAIL=admin@vitalplatforms.com ADMIN_PASSWORD='StrongPass' ./platform-manager.sh deploy-remote
  CODEBASE_SOURCE=git GIT_REPO_URL=https://github.com/HISP-Uganda/dhis2-superset.git GIT_BRANCH=martbase DOMAIN=supersets.vitalplatforms.com ./platform-manager.sh deploy-remote
  CODEBASE_SOURCE=git GIT_REPO_URL=https://github.com/HISP-Uganda/dhis2-superset.git GIT_REF=martbase DOMAIN=supersets.vitalplatforms.com ./platform-manager.sh upgrade-remote
  ./platform-manager.sh start-all
EOF
}
