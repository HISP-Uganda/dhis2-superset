...
AI_INSIGHTS_REQUEST_TIMEOUT_SECONDS=120
AI_INSIGHTS_MAX_TOKENS=8192
...
AI_INSIGHTS_CONFIG = {
    ...
    "request_timeout_seconds": env_int("AI_INSIGHTS_REQUEST_TIMEOUT_SECONDS", 120),
    "max_tokens": env_int("AI_INSIGHTS_MAX_TOKENS", 8192),
...
patch_metadata_db_schema_for_dhis2() {
  ...
  local has_dbs
  has_dbs="$(sudo -u postgres psql -d "$POSTGRES_DB" -tAc "SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='dbs' LIMIT 1;")"
  if [[ "$has_dbs" != "1" ]]; then
