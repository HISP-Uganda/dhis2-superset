...
function quoteSqlIdentifier(part?: string | null) {
...
function buildSelectStarQuery(
  tableName: string,
  catalogName?: string | null,
  schemaName?: string,
) {
  const tableRef = [catalogName, schemaName, tableName]
...
function isDefaultEditorSql(sql?: string) {
  const normalizedSql = normalizeSql(sql);
  return (
    !normalizedSql ||
    normalizedSql.endsWith('SELECT ...') ||
    normalizedSql === 'SELECT * FROM WHERE' ||
    normalizedSql === 'SELECT * FROM WHERE LIMIT'
  );
}
...
const SqlEditorLeftBar = ({
  ...
  const autoGeneratedSqlBySelectedTable = useMemo(
    () =>
      new Set(
        tables.map(({ name, catalog, schema }) =>
          normalizeSql(buildSelectStarQuery(name, catalog, schema)),
        ),
      ),
    [tables],
  );
  ...
  const onTablesChange = (
    ...
    if (tablesToAdd.length === 1 && queryEditor) {
      const selectedTable = tablesToAdd[0];
      const selectedTableName =
        typeof selectedTable === 'string' ? selectedTable : selectedTable.value;
      const nextSql = buildSelectStarQuery(
        selectedTableName,
        catalogName,
        schemaName,
      );
      ...
      if (
        isDefaultEditorSql(currentSql) ||
        autoGeneratedSqlBySelectedTable.has(normalizeSql(currentSql))
      ) {
        dispatch(queryEditorSetAndSaveSql(queryEditor, nextSql));
      }
    }
    ...
    tablesToAdd.forEach(tableOrName => {
      ...
      const tableLabel =
        typeof tableOrName === 'string' ? undefined : tableOrName.label;
  ...
  const handleDbList = useCallback(
    (result: DatabaseObject[] = []) => {
      ...
      const currentDbId = Number(queryEditor.dbId);
      const currentDb =
        result.find(database => database.id === currentDbId) || result[0];

      if (!userSelectedDb || userSelectedDb.id !== currentDb.id) {
        setUserSelected(currentDb);
      }

      const hasValidCurrentDb =
        Number.isFinite(currentDbId) &&
        result.some(database => database.id === currentDbId);
      if (!hasValidCurrentDb) {
        dispatch(queryEditorSetCatalog(queryEditor, undefined));
        dispatch(queryEditorSetSchema(queryEditor, undefined));
        dispatch(queryEditorSetDb(queryEditor, currentDb.id));
      }
    },
    [dispatch, queryEditor, userSelectedDb],
  );
