/**
 * Licensed to the Apache Software Foundation (ASF) under one
 * or more contributor license agreements.  See the NOTICE file
 * distributed with this work for additional information
 * regarding copyright ownership.  The ASF licenses this file
 * to you under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance
 * with the License.  You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 * KIND, either express or implied.  See the License for the
 * specific language governing permissions and limitations
 * under the License.
 */
import '@testing-library/jest-dom';
import { render, screen, waitFor, within } from '@testing-library/react';
import userEvent from '@testing-library/user-event';
import { MemoryRouter } from 'react-router-dom';

const mockAddDangerToast = jest.fn();
const mockAddInfoToast = jest.fn();
const mockAddSuccessToast = jest.fn();

jest.mock('@superset-ui/core', () => {
  const React = require('react');

  const createStyledComponent = (Component: any) => () =>
    React.forwardRef(({ children, ...props }: any, ref: any) =>
      React.createElement(Component, { ref, ...props }, children),
    );

  return {
    __esModule: true,
    SupersetClient: {
      get: jest.fn(),
      post: jest.fn(),
      put: jest.fn(),
      delete: jest.fn(),
    },
    css: () => '',
    styled: new Proxy(createStyledComponent('div'), {
      apply: (_target, _thisArg, argList) => createStyledComponent(argList[0]),
      get: (_target, prop) => createStyledComponent(prop),
    }),
    t: (template: string, ...args: Array<string | number>) =>
      args.reduce<string>(
        (message, arg) => message.replace('%s', String(arg)),
        template,
      ),
  };
});

jest.mock('@superset-ui/core/components', () => ({
  __esModule: true,
  Typography: {
    Title: ({ children }: any) => <h1>{children}</h1>,
    Paragraph: ({ children }: any) => <p>{children}</p>,
    Text: ({ children }: any) => <span>{children}</span>,
  },
  Loading: () => <div>Loading</div>,
}));

jest.mock('src/features/home/SubMenu', () => ({
  __esModule: true,
  default: ({ name, tabs }: any) => (
    <div>
      <div>{name}</div>
      {tabs?.map((tab: any) => (
        <span key={tab.name}>{tab.label}</span>
      ))}
    </div>
  ),
}));

jest.mock('src/components/MessageToasts/withToasts', () => ({
  useToasts: () => ({
    addDangerToast: mockAddDangerToast,
    addInfoToast: mockAddInfoToast,
    addSuccessToast: mockAddSuccessToast,
  }),
}));

import { SupersetClient } from '@superset-ui/core';
import DHIS2Instances from '.';

const mockClient = SupersetClient as jest.Mocked<typeof SupersetClient>;

beforeEach(() => {
  jest.clearAllMocks();

  mockClient.get.mockImplementation(async ({ endpoint }: { endpoint: string }) => {
    if (endpoint.startsWith('/api/v1/database/')) {
      return {
        json: {
          count: 1,
          result: [
            {
              id: 7,
              database_name: 'DHIS2 Uganda',
              backend: 'dhis2',
            },
          ],
        },
      } as any;
    }
    if (endpoint.includes('/api/v1/dhis2/instances/')) {
      return {
        json: {
          count: 1,
          result: [
            {
              id: 1,
              database_id: 7,
              name: 'Uganda HMIS',
              url: 'https://uganda.example.org',
              auth_type: 'basic',
              username: 'admin',
              is_active: true,
              description: 'Primary production federation',
              changed_on: '2026-03-12T08:00:00',
            },
          ],
        },
      } as any;
    }
    throw new Error(`Unexpected GET ${endpoint}`);
  });

  mockClient.post.mockImplementation(async ({ endpoint }: { endpoint: string }) => {
    if (endpoint === '/api/v1/dhis2/instances/1/test') {
      return {
        json: {
          result: {
            success: true,
            message: 'Connected in 120ms',
            response_time_ms: 120,
          },
        },
      } as any;
    }
    if (endpoint === '/api/v1/dhis2/instances/') {
      return {
        json: {
          result: {
            id: 2,
            name: 'Kenya HMIS',
          },
        },
      } as any;
    }
    if (endpoint === '/api/v1/dhis2/instances/migrate-legacy') {
      return { json: { result: null } } as any;
    }
    throw new Error(`Unexpected POST ${endpoint}`);
  });
});

test('renders existing instances and supports testing and creating one', async () => {
  render(
    <MemoryRouter initialEntries={['/superset/dhis2/instances/']}>
      <DHIS2Instances />
    </MemoryRouter>,
  );

  expect(await screen.findByText('Uganda HMIS')).toBeInTheDocument();

  await userEvent.click(
    screen.getByRole('button', { name: 'Test Connection' }),
  );
  expect(await screen.findByText('Connection OK')).toBeInTheDocument();

  await userEvent.click(screen.getByRole('button', { name: 'Add Instance' }));
  const dialog = await screen.findByRole('dialog');

  await userEvent.type(within(dialog).getByLabelText('Name'), 'Kenya HMIS');
  await userEvent.type(
    within(dialog).getByPlaceholderText('https://hmis.example.org'),
    'https://kenya.example.org',
  );
  await userEvent.type(within(dialog).getByLabelText('Username'), 'api_user');
  await userEvent.type(
    within(dialog).getByLabelText('Password'),
    'secret123',
  );
  await userEvent.click(within(dialog).getByRole('button', { name: 'Save' }));

  await waitFor(() => {
    expect(mockClient.post).toHaveBeenCalledWith(
      expect.objectContaining({
        endpoint: '/api/v1/dhis2/instances/',
        jsonPayload: expect.objectContaining({
          auth_type: 'basic',
          database_id: 7,
          name: 'Kenya HMIS',
          password: 'secret123',
          url: 'https://kenya.example.org',
          username: 'api_user',
        }),
      }),
    );
  });
});
