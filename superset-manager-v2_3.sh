...
ensure_remote_app_user() {
  ...
  if [[ "$REMOTE_USER" == "root" ]]; then
    ...
    remote_exec "printf '%s\n' '${REMOTE_APP_USER} ALL=(ALL) NOPASSWD:ALL' > /etc/sudoers.d/${REMOTE_APP_USER}"
    remote_exec "chmod 440 /etc/sudoers.d/${REMOTE_APP_USER}"
...
Remote deployment:
  Note: when REMOTE_USER=root, the script grants passwordless sudo to REMOTE_APP_USER so remote install can complete non-interactively.
